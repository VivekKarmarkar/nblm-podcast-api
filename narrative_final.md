...
A few minutes later, a computer hands you back a picture of what's inside the cube. And there's the clay. Right where someone hid it.
...
So here's the pipeline. You take the video. You extract those sub-pixel motions. Then you do a frequency analysis — a way of separating a tangled wiggly signal into the pure pitches it's made of, kind of like how your ear can pick out individual notes in a chord. You find the frequencies at which the object likes to wobble, and at each of those frequencies, you sketch out the wobble pattern. Out comes a list of mode shapes. Now you ask the computer: what material map, what arrangement of stiffness and density inside this object, would have produced exactly these modes?

That last step is where the real work lives. It's a kind of guess-and-check, just sped up enormously. The computer starts with a guess for the interior material map, works out what modes that material would produce, compares to the observed modes, adjusts the guess, tries again, compares again. Over and over until it lands on a material map that matches what the video actually saw.
...
All of which means the math problem is what's called ill-posed. There isn't a unique answer. Lots of different interior material maps would produce roughly the same observed modes. To pick one, the method adds a preference: it prefers material maps that vary smoothly in space. The idea being, real defects show up as compact connected regions, not as random salt-and-pepper noise. With that smoothness preference, the math settles on a unique answer — usually the right one.

That's the method. Tiny video motions, decomposed into mode patterns, run backwards through a computer that grinds away with a smoothness preference baked in. Material map out the other end.

Does it actually work? They tested it three ways, and the results get progressively more amazing.

First, they did what's called a simulation — a computer rehearsal of the physics where we already know the right answer ahead of time, so we can check if the method gets the right answer. They made virtual cubes — fake Jello, with a fake clay defect placed somewhere inside — and let the computer crunch out what the surface would do under the laws of physics. Then they ran their pipeline on that simulated surface motion and asked: did we recover the defect? Yes. With enough modes, the defect appears in the right place with roughly the right shape. With fewer modes, you get a vaguer blob, but still in the right neighborhood. And — this is a nice realism check — defects near the bottom of the cube are harder to find than defects near the top, because the bottom is fixed in place and barely moves. The math can only see what's actually shaking.
...
A cube of Jello sitting on a table. A regular phone. A tiny nudge. A few seconds of video. And the computer finds the clay.
